...
export function EditRoleDialog({
  ...
  useEffect(() => {
    if (role) {
      ...
      setBypassEnabled(role.byPassAllFeatures);
    }
  }, [role, form]);

  const updateMutation = useMutation({
    mutationFn: async (data: EditRoleInput) => {
      const response = await axiosInstance.put(`/api/roles/${role?.id}`, data);
      return response;
    },
    onSuccess: () => {
  ...
  const handleSubmit = (data: EditRoleInput) => {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update role dan permission yang sesuai
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
